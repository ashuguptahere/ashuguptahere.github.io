...
def build_stats_cards(links: list[dict]) -> list[str]:
    ...
    if github:
        ...
        for alt, path, extra in (
            ...
            cards.append(themed(alt, f"{stem}&{LIGHT_INK}", f"{stem}&{DARK_INK}", github))

        # Contribution snake, generated by the Platane/snk action in the
        # profile repo; it publishes a light and a dark variant.
        snake = f"https://raw.githubusercontent.com/{user}/{user}/output/github-contribution-grid-snake"
        cards.append(
            themed(
                "Contribution snake",
                f"{snake}.svg",
                f"{snake}-dark.svg",
                f"https://github.com/{user}/{user}",
            )
        )
